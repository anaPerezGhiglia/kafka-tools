...
class KafkaToolsTest extends FlatSpec {
  ...
  "KafkaTools#newBrokersLoad" should "change load only of the affectedBrokers" in {
    val brokersLoad = Map(1 -> 3, 2 -> 4, 3 -> 3, 4 -> 5)
    val affectedBrokers = List(2, 4)

    val newBrokersLoad = KafkaTools.newBrokersLoad(brokersLoad, affectedBrokers, originalLoad => originalLoad - 1)
    assertResult(Map(1 -> 3, 2 -> 3, 3 -> 3, 4 -> 4))(newBrokersLoad)
  }

  "KafkaTools#brokersLoad" should "only take into cosideration the load of partitions to be reassigned" in {
    ...
    val brokersLoad = KafkaTools.calculateBrokersLoad(brokers, partitionsByBroker, partitionsToBeReassigned)
